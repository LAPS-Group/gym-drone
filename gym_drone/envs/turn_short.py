...
class _TurnShort():
    ...
    def _vector_angle(a, b, c):
        ...
        angle = math.acos(max(min(dot_product / (BA_magnitude * BC_magnitude), 1), 0))
        return angle
    ...
    def _line_sampled_height(start, stop, grid, stepsize=0.1):
        ...
        unit_vector = tuple(map(operator.truediv, vector, [vector_magnitude]*2))
    ...
    def _circle_sampled_height(start, stop, origo, grid, stepsize=0.1):
        ...
        rad_start = _TurnShort._vector_angle(start, origo, O_i)
        rad_stop = _TurnShort._vector_angle(stop, origo, O_i)
    ...
    def flight_possible(a, b, c, turnradius):
        ...
        angle = _TurnShort._vector_angle(a, b, c) / 2
    ...
    def shortest_turn(a, b, c, turnradius):
        # angle from tangent lines to circle center, divided by two as it is
        # in the center of the angle between the two vectors.
        relative_angle = _TurnShort._vector_angle(a, b, c) / 2
        ...
        BA_angle = _TurnShort._vector_angle(B_i, b, a)
        BC_angle = _TurnShort._vector_angle(B_i, b, c)
        lowest_angle_vector = a
        if BC_angle < BA_angle:
            lowest_angle_vector = c

        absolute_angle = _TurnShort._vector_angle(B_i, b, lowest_angle_vector)
        ...
        BA_magnitude = np.linalg.norm(BA)
        BC_magnitude = np.linalg.norm(BC)
        BA = tuple(map(operator.mul, [tangent_distance / BA_magnitude] * 2, BA))
        BC = tuple(map(operator.mul, [tangent_distance / BC_magnitude] * 2, BC))
    ...
    def get_turn_height(a, b, c, turn_rate, grid, stepsize=0.1):
        turn = _TurnShort.shortest_turn(a, b, c, turn_rate)
        ...
        pre_turn_height = _TurnShort._line_sampled_height(a, turn['A'], grid)
        turn_height = _TurnShort._circle_sampled_height(turn['A'],
                                                        turn['C'],
                                                        turn['O'],
                                                        grid)
        post_turn_height = _TurnShort._line_sampled_height(turn['C'], c, grid)
...
class TurnShortEnv(gym.Env):
    ...
    def step(self, action):
        ...
        new_position = action_to_position(action, self._shape, self._subsampling)
        ...
        total_height = _TurnShort.get_turn_height(self._drone_pos,
                                                  new_position,
                                                  self._goal_pos,
                                                  self._turn_rate,
                                                  self._heightmap)
        ...
        # total height member set, to make making your own reward function
        # easier, outside of this python module
        ...
        # TODO: revise this function, as it is as of now relatively arbitrary.
        reward = max(10 - total_height / 10000, 0)
        episode_over = True
