...
class TurnShort:
    ...
    def _line_sampled_height(self, start, stop, grid, stepsize=0.1):
    ...
    def _circle_sampled_height(self, start, stop, origo, grid, stepsize=0.1):
        ...
        rad_start = self._vector_angle(start, origo, O_i)
        rad_stop = self._vector_angle(stop, origo, O_i)
    ...
    def flight_possible(self, a, b, c, turnradius):
        BA = tuple(map(operator.sub, a, b))
        BC = tuple(map(operator.sub, c, b))
        BA_magnitude = np.linalg.norm(BA)
        BC_magnitude = np.linalg.norm(BC)
        ...
        angle = self._vector_angle(a, b, c) / 2
    ...
    # while b is the center point which determines the 
    # angle that tangent ac
    def shortest_turn(self, a, b, c, turnradius):
        # angle from tangent lines to circle center, divided by two as it is
        # in the center of the angle between the two vectors.
        relative_angle = self._vector_angle(a, b, c) / 2
        ...
        BA_angle = self._vector_angle(B_i, b, a)
        BC_angle = self._vector_angle(B_i, b, c)
        lowest_angle_vector = a
        if BC_angle < BA_angle:
            lowest_angle_vector = c

        absolute_angle = self._vector_angle(B_i, b, lowest_angle_vector)
    ...
    def get_turn_height(self, a, b, c, turn_rate, grid, stepsize=0.1):
        turn = self.shortest_turn(a, b, c, turn_rate)
        ...
        pre_turn_height = self._line_sampled_height(a, turn['A'], grid)
        turn_height = self._circle_sampled_height(turn['A'],
                                                        turn['C'],
                                                        turn['O'],
                                                        grid)
        post_turn_height = self._line_sampled_height(turn['C'], c, grid)
